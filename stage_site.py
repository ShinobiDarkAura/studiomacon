...
def main():
    ...
    # version the admin's own assets — it isn't generated by build.py, so without
    # this a stale admin.js can sit in a browser cache for the max-age window
    admin_html = os.path.join(OUT, "admin", "index.html")
    if os.path.exists(admin_html):
        import hashlib as _h
        s = open(admin_html, encoding="utf-8").read()
        for asset in ("admin.js", "admin.css"):
            p = os.path.join(OUT, "admin", asset)
            if os.path.exists(p):
                ver = _h.sha1(open(p, "rb").read()).hexdigest()[:8]
                s = s.replace(f'"{asset}"', f'"{asset}?v={ver}"')
        open(admin_html, "w", encoding="utf-8").write(s)
        print("versioned admin assets")
